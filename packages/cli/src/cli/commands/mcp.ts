...
import { addServer, removeServer, listServers } from "@purix/core/mcp/mcp_registry";
import { listServerTools, callServerTool } from "@purix/core/mcp/mcp_gateway";

export function registerMcpCommands(program: Command) {
  // ---------------------------------------------------------------------------
  // MCP Gateway
  // ---------------------------------------------------------------------------
  program.command("mcp-add <n> <url>").description("Register an MCP server").action((name: string, url: string) => {
    addServer(name, url);
    console.log(`✅ Registered MCP server "${name}" -> ${url}`);
  });

  program.command("mcp-remove <n>").description("Remove a registered MCP server").action((name: string) => {
    console.log(removeServer(name) ? `✅ Removed "${name}".` : `No server registered as "${name}".`);
  });

  program.command("mcp-list").description("List registered MCP servers").action(() => {
